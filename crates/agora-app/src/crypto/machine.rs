...
impl CryptoMachine {
    ...
    pub fn encrypt_olm(
        ...
        self.store
            .data
            .olm_sessions
            .insert(recipient_curve_key.to_owned(), vec![pickle_olm_session(&session)]);
        self.store.save().ok();
    ...
    pub fn decrypt_megolm(
        ...
        let plaintext_str = String::from_utf8(result.plaintext)
            .map_err(|e| format!("invalid utf8: {e}"))?;
        let payload: DecryptedPayload = serde_json::from_str(&plaintext_str)
